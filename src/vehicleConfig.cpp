...
#define CONFIG_HAS_SBUS // serial connection uses SBUS protocol instead of normal protocol, if not commented out
...
/*
  // Battery type
  bool liPo; // If "true", the vehicle can't be reactivated once the cutoff voltage is reached
  ...
  bool HP; // HP = "High Power" version (both TB6612FNG channels wired in parallel) -> No motor 1, motor 2 is the driving motor
  ...
  1 = vhctype_semi caterpillar, 2 = caterpillar (see: https://www.youtube.com/watch?v=Tjikm6hJ8hQ)
  ...
  bool tailLights; // Caution: the taillights are wired to the servo pin 2! -> Servo 2 not usable, if "true"
  bool headLights; // Caution: the headlights are wired to the RXI pin! -> Serial not usable, if "true"
  bool indicators; // Caution: the indicators are wired to the SDA / SCL pins! -> not usable, if vehicleType is 4 or 5 (locked)
  bool beacons; // Caution: the beacons are wired to the servo pin 4! -> Servo 4 not usable, if "true"
  ...
  // Additional Channels
  bool TXO_momentary1; // The TXO output is linked to the momentary1 channel! -> Serial not usable, if "true"
  bool TXO_toggle1; // The TXO output is linked to the toggle1 channel! -> Serial not usable, if "true"
  bool potentiometer1; // The potentiometer knob on the transmitter is linked to the servo output CH4

  // Engine sound (see: https://www.youtube.com/watch?v=pPlrx9yVI6E)
  bool engineSound; // true = a "TheDIYGuy999" engine simulator is wired to servo channel 3 (allows to switch off RC signal, deprecated)

  // Tone sound (see: https://www.youtube.com/watch?v=fe5_1mMtcLQ&t=3s)
  bool toneOut; // true = a BC337 amplifier for tone() is connected instead of servo 3
*/
...
void vehicleConfig::begin(vehicleconfig_t vehicle)
{
  switch (vehicle)
  {
    ...
    case STANDARDCAR:
    {
      setBasicParams(false,3.1,1.0,false,1,car,false,false);
      setIndicators(false,false,false,false,false);
      setServoLimits(45,135,45,135,45,135,75,105,45,135);
      setMotorConfig(255,170,0,3,12,0,255,32);
    }
    break;
  }
}

void vehicleConfig::setBasicParams(
      bool liPo,
      float cutoffVoltage,
      float boardVersion,
      bool HP,
  ...
  void vehicleConfig::setServoLimits(
      byte lim1L,
      byte lim1R,
      byte lim2L,
      byte lim2R,
  ...
  void vehicleConfig::setIndicators(
      bool escBrakeLights,
      bool tailLights,
      bool headLights,
      bool indicators,
    ...
    {
      m_escBrakeLights = escBrakeLights;
  ...
  void vehicleConfig::setMotorConfig(
      int maxPWMfull,
      int maxPWMlimited,
      int minPWM,
  ...
  {
    m_maxPWMfull = maxPWMfull;
    m_maxPWMlimited = maxPWMlimited;
    m_minPWM = minPWM;
    m_maxAccelerationFull = maxAccelerationFull;
    m_maxAccelerationLimited = maxAccelerationLimited;
    m_tiltCalibration = tiltCalibration;
    m_steeringTorque = steeringTorque;
    m_pwmPrescaler2 = pwmPrescaler2;
  }

  void vehicleConfig::setAdditionalChannels(
      bool TXO_momentary1,
      bool TXO_toggle1,
      bool potentiometer1
  )
  {
    m_TXO_momentary1 = TXO_momentary1;
    m_TXO_toggle1 = TXO_toggle1;
    m_potentiometer1 = potentiometer1;
  }
...
// // Battery type
// bool liPo = false; // ESC provides protection
...
// // Battery type
// bool liPo = false; // LiPo is protected by ESC
...
// // Battery type
// bool liPo = false; // protected by the ESC
...
// bool TXO_momentary1 = true;
// bool TXO_toggle1 = false;
// bool potentiometer1 = true; // true = MRSC knob linked to servo CH4!
